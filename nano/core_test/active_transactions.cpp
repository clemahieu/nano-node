...
TEST (inactive_votes_cache, basic)
{
	...
	ASSERT_TIMELY (5s, node.ledger.confirmed.exists_or_pruned (node.store.tx_begin_read (), send->hash ()));
...
TEST (active_transactions, confirmation_consistency)
{
	...
	for (unsigned i = 0; i < 10; ++i)
	{
		...
		while (!node.ledger.confirmed.exists_or_pruned (node.store.tx_begin_read (), block->hash ()))
		{
			node.scheduler.priority.activate (nano::dev::genesis_key.pub, node.store.tx_begin_read ());
