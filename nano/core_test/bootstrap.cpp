...
TEST (bootstrap_processor, push_diamond_pruning)
{
...

	{
		...
		ASSERT_FALSE (node1->ledger.any.exists (transaction, send1->hash ()));
		ASSERT_TRUE (node1->store.pruned.exists (transaction, send1->hash ()));
		ASSERT_FALSE (node1->ledger.any.exists (transaction, open->hash ()));
		ASSERT_TRUE (node1->store.pruned.exists (transaction, open->hash ()));
		ASSERT_TRUE (node1->ledger.any.exists (transaction, send2->hash ()));
		ASSERT_TRUE (node1->ledger.any.exists (transaction, receive->hash ()));
