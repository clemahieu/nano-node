...
TEST (ledger, empty)
{
	...
	auto balance (ledger.any.balance (transaction, account));
...
TEST (ledger, genesis_balance)
{
	...
	auto balance = ledger.any.balance (transaction, nano::dev::genesis_key.pub);
	ASSERT_EQ (nano::dev::constants.genesis_amount, balance);
	auto info = ledger.any.get (transaction, nano::dev::genesis_key.pub);
	ASSERT_TRUE (info);
...
TEST (ledger, process_modifies_sideband)
{
	...
	ASSERT_EQ (send1->sideband ().timestamp, ledger.any.get (store.tx_begin_read (), send1->hash ())->sideband ().timestamp);
}
...
TEST (ledger, process_send)
{
	...
	auto transaction = store.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto info1 = ledger.any.get (transaction, nano::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	nano::keypair key2;
	nano::block_builder builder;
	auto send = builder
	...
	ASSERT_EQ (nano::dev::constants.genesis_amount - 50, ledger.any.amount (transaction, hash1));
	ASSERT_EQ (nano::dev::genesis_key.pub, send->account ());
	ASSERT_EQ (50, ledger.any.balance (transaction, nano::dev::genesis_key.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount - 50, ledger.account_receivable (transaction, key2.pub));
	auto info2 = ledger.any.get (transaction, nano::dev::genesis_key.pub);
	ASSERT_TRUE (info2);
	ASSERT_EQ (2, info2->block_count);
	auto latest6 = ledger.any.get (transaction, info2->head);
	...
	ASSERT_EQ (nano::dev::constants.genesis_amount - 50, ledger.any.amount (transaction, hash2));
	ASSERT_EQ (nano::block_status::progress, return2);
	ASSERT_EQ (key2.pub, open->account ());
	ASSERT_EQ (nano::dev::constants.genesis_amount - 50, ledger.any.amount (transaction, hash2));
	ASSERT_EQ (nano::dev::constants.genesis_amount - 50, ledger.any.balance (transaction, key2.pub));
	...
	auto info3 = ledger.any.get (transaction, nano::dev::genesis_key.pub);
	ASSERT_TRUE (info3);
	auto latest2 = ledger.any.get (transaction, info3->head);
	ASSERT_NE (nullptr, latest2);
	auto latest3 = dynamic_cast<nano::send_block *> (latest2.get ());
	ASSERT_NE (nullptr, latest3);
	ASSERT_EQ (*send, *latest3);
	auto info4 = ledger.any.get (transaction, key2.pub);
	ASSERT_TRUE (info4);
	auto latest4 = ledger.any.get (transaction, info4->head);
	...
	auto info5 = ledger.any.get (transaction, key2.pub);
	ASSERT_FALSE (info5);
	auto pending1 = ledger.any.get (transaction, nano::pending_key (key2.pub, hash1));
	...
	ASSERT_FALSE (ledger.any.balance (transaction, key2.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount - 50, ledger.account_receivable (transaction, key2.pub));
	ASSERT_EQ (50, ledger.any.balance (transaction, nano::dev::genesis_key.pub).value ());
	ASSERT_EQ (50, ledger.weight (nano::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (key2.pub));
	auto info6 = ledger.any.get (transaction, nano::dev::genesis_key.pub);
	ASSERT_TRUE (info6);
	...
	auto info7 = ledger.any.get (transaction, nano::dev::genesis_key.pub);
	ASSERT_TRUE (info7);
	ASSERT_EQ (1, info7->block_count);
	ASSERT_EQ (info1->head, info7->head);
	ASSERT_FALSE (ledger.any.get (transaction, nano::pending_key (key2.pub, hash1)));
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.any.balance (transaction, nano::dev::genesis_key.pub).value ());
	ASSERT_EQ (0, ledger.account_receivable (transaction, key2.pub));
...
TEST (ledger, process_receive)
{
	...
	ASSERT_EQ (25, ledger.any.amount (transaction, hash4));
	ASSERT_EQ (nano::block_status::progress, return2);
	ASSERT_EQ (key2.pub, receive->account ());
	ASSERT_EQ (hash4, ledger.any.head (transaction, key2.pub));
	ASSERT_EQ (25, ledger.any.balance (transaction, nano::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.account_receivable (transaction, key2.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount - 25, ledger.any.balance (transaction, key2.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount - 25, ledger.weight (key3.pub));
	ASSERT_FALSE (ledger.rollback (transaction, hash4));
	ASSERT_FALSE (ledger.any.successor (transaction, hash2));
	ASSERT_EQ (25, ledger.any.balance (transaction, nano::dev::genesis_key.pub));
	ASSERT_EQ (25, ledger.account_receivable (transaction, key2.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount - 50, ledger.any.balance (transaction, key2.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount - 50, ledger.weight (key3.pub));
	ASSERT_EQ (hash2, ledger.any.head (transaction, key2.pub));
	auto pending1 = ledger.any.get (transaction, nano::pending_key (key2.pub, hash3));
	ASSERT_TRUE (pending1);
	ASSERT_EQ (nano::dev::genesis_key.pub, pending1->source);
...
TEST (ledger, rollback_receiver)
{
	...
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.any.balance (transaction, nano::dev::genesis_key.pub));
	ASSERT_FALSE (ledger.any.balance (transaction, key2.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.weight (nano::dev::genesis_key.pub));
	ASSERT_EQ (0, ledger.weight (key2.pub));
	ASSERT_EQ (0, ledger.weight (key3.pub));
	ASSERT_FALSE (ledger.any.get (transaction, key2.pub));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
	ASSERT_FALSE (ledger.any.get (transaction, nano::pending_key{ key2.pub, hash1 }));
}
...
TEST (ledger, rollback_representation)
{
	...
	auto info4 = ledger.any.get (transaction, nano::dev::genesis_key.pub);
	ASSERT_TRUE (info4);
...
TEST (ledger, representative_genesis)
{
	...
	auto & store = ctx.store ();
	auto transaction = store.tx_begin_write ();
	auto latest = ledger.any.head (transaction, nano::dev::genesis_key.pub);
...
TEST (ledger, representative_change)
{
	...
	ASSERT_EQ (nano::block_status::progress, return1);
	ASSERT_EQ (0, ledger.any.amount (transaction, block->hash ()));
...
TEST (ledger, successor)
{
	...
	auto transaction (node1.store.tx_begin_write ());
	ASSERT_EQ (nano::block_status::progress, node1.ledger.process (transaction, send1));
	ASSERT_EQ (*send1, *node1.ledger.any.get (transaction, node1.ledger.any.successor (transaction, nano::qualified_root (nano::root (0), nano::dev::genesis->hash ())).value ()));
	ASSERT_EQ (*nano::dev::genesis, *node1.ledger.any.get (transaction, node1.ledger.any.successor (transaction, nano::dev::genesis->qualified_root ()).value ()));
	ASSERT_FALSE (node1.ledger.any.successor (transaction, nano::qualified_root (0)));
}
...
TEST (ledger, latest_empty)
{
	...
	nano::keypair key;
	auto transaction = store.tx_begin_read ();
	auto latest = ledger.any.head (transaction, key.pub);
...
TEST (ledger, latest_root)
{
	...
	auto hash1 = ledger.any.head (transaction, nano::dev::genesis_key.pub);
	nano::block_builder builder;
	auto send = builder
				.send ()
				.previous (hash1)
...
TEST (ledger, block_destination_source)
{
	...
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, block5));
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, block6));
	ASSERT_EQ (balance, ledger.any.balance (transaction, block6->hash ()));
...
TEST (ledger, state_account)
{
	...
	auto send1 = builder
				 ...
				 .build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, send1));
	ASSERT_EQ (nano::dev::genesis_key.pub, ledger.any.account (transaction, send1->hash ()));
}
...
TEST (ledger, state_send_receive)
{
	...
	auto send1 = builder
				 ...
				 .build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, send1));
	ASSERT_TRUE (ledger.any.exists (transaction, send1->hash ()));
	auto send2 = ledger.any.get (transaction, send1->hash ());
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gxrb_ratio, ledger.any.balance (transaction, send1->hash ()));
	ASSERT_EQ (nano::Gxrb_ratio, ledger.any.amount (transaction, send1->hash ()));
	...
	auto receive1 = builder
					...
					.build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_TRUE (ledger.any.exists (transaction, receive1->hash ()));
	auto receive2 = ledger.any.get (transaction, receive1->hash ());
	ASSERT_NE (nullptr, receive2);
	ASSERT_EQ (*receive1, *receive2);
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.any.balance (transaction, receive1->hash ()));
	ASSERT_EQ (nano::Gxrb_ratio, ledger.any.amount (transaction, receive1->hash ()));
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.weight (nano::dev::genesis_key.pub));
	ASSERT_FALSE (ledger.any.get (transaction, nano::pending_key{ nano::dev::genesis_key.pub, send1->hash () }));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
...
TEST (ledger, state_rep_change)
{
	...
	auto change1 = builder
				   ...
				   .build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, change1));
	ASSERT_TRUE (ledger.any.exists (transaction, change1->hash ()));
	auto change2 = ledger.any.get (transaction, change1->hash ());
	ASSERT_NE (nullptr, change2);
	ASSERT_EQ (*change1, *change2);
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.any.balance (transaction, change1->hash ()));
	ASSERT_EQ (0, ledger.any.amount (transaction, change1->hash ()));
	ASSERT_EQ (0, ledger.weight (nano::dev::genesis_key.pub));
...
TEST (ledger, state_open)
{
	...
	ASSERT_EQ (nano::Gxrb_ratio, ledger.any.amount (transaction, send1->hash ()));
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gxrb_ratio, ledger.weight (nano::dev::genesis_key.pub));
	ASSERT_TRUE (ledger.any.get (transaction, nano::pending_key{ destination.pub, send1->hash () }));
	auto open1 = builder
				 .state ()
				 ...
				 .build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, open1));
	ASSERT_FALSE (ledger.any.get (transaction, nano::pending_key{ destination.pub, send1->hash () }));
	ASSERT_TRUE (ledger.any.exists (transaction, open1->hash ()));
	auto open2 = ledger.any.get (transaction, open1->hash ());
	ASSERT_NE (nullptr, open2);
	ASSERT_EQ (*open1, *open2);
	ASSERT_EQ (nano::Gxrb_ratio, ledger.any.balance (transaction, open1->hash ()));
	ASSERT_EQ (nano::Gxrb_ratio, ledger.any.amount (transaction, open1->hash ()));
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.weight (nano::dev::genesis_key.pub));
...
TEST (ledger, state_receive_old)
{
	...
	auto receive1 = builder
					...
					.build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_EQ (2 * nano::Gxrb_ratio, ledger.any.balance (transaction, receive1->hash ()));
	ASSERT_EQ (nano::Gxrb_ratio, ledger.any.amount (transaction, receive1->hash ()));
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.weight (nano::dev::genesis_key.pub));
}
...
TEST (ledger, state_rollback_send)
{
	...
	ASSERT_NE (nullptr, send2);
	ASSERT_EQ (*send1, *send2);
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gxrb_ratio, ledger.any.balance (transaction, nano::dev::genesis_key.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gxrb_ratio, ledger.weight (nano::dev::genesis_key.pub));
	auto info = ledger.any.get (transaction, nano::pending_key (nano::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info);
	ASSERT_EQ (nano::dev::genesis_key.pub, info->source);
	ASSERT_EQ (nano::Gxrb_ratio, info->amount.number ());
	ASSERT_FALSE (ledger.rollback (transaction, send1->hash ()));
	ASSERT_FALSE (ledger.any.exists (transaction, send1->hash ()));
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.any.balance (transaction, nano::dev::genesis_key.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.weight (nano::dev::genesis_key.pub));
	ASSERT_FALSE (ledger.any.get (transaction, nano::pending_key{ nano::dev::genesis_key.pub, send1->hash () }));
	ASSERT_FALSE (ledger.any.successor (transaction, nano::dev::genesis->hash ()));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}
...
TEST (ledger, state_rollback_receive)
{
	...
	auto receive1 = builder
					...
					.build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, receive1));
	ASSERT_FALSE (ledger.any.get (transaction, nano::pending_key{ nano::dev::genesis_key.pub, receive1->hash () }));
	ASSERT_FALSE (ledger.rollback (transaction, receive1->hash ()));
	auto info = ledger.any.get (transaction, nano::pending_key (nano::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info);
	ASSERT_EQ (nano::dev::genesis_key.pub, info->source);
	ASSERT_EQ (nano::Gxrb_ratio, info->amount.number ());
	ASSERT_FALSE (ledger.any.exists (transaction, receive1->hash ()));
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gxrb_ratio, ledger.any.balance (transaction, nano::dev::genesis_key.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gxrb_ratio, ledger.weight (nano::dev::genesis_key.pub));
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
}
...
TEST (ledger, state_rollback_received_send)
{
	...
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.any.balance (transaction, nano::dev::genesis_key.pub).value ());
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.weight (nano::dev::genesis_key.pub));
	ASSERT_FALSE (ledger.any.balance (transaction, key.pub));
...
TEST (ledger, state_rep_change_rollback)
{
	...
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, change1));
	ASSERT_FALSE (ledger.rollback (transaction, change1->hash ()));
	ASSERT_FALSE (ledger.any.exists (transaction, change1->hash ()));
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.any.balance (transaction, nano::dev::genesis_key.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount, ledger.weight (nano::dev::genesis_key.pub));
...
TEST (ledger, state_open_rollback)
{
	...
	ASSERT_FALSE (ledger.any.exists (transaction, open1->hash ()));
	ASSERT_FALSE (ledger.any.balance (transaction, destination.pub));
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gxrb_ratio, ledger.weight (nano::dev::genesis_key.pub));
	auto info = ledger.any.get (transaction, nano::pending_key (destination.pub, send1->hash ()));
	ASSERT_TRUE (info);
	ASSERT_EQ (nano::dev::genesis_key.pub, info->source);
	ASSERT_EQ (nano::Gxrb_ratio, info->amount.number ());
	ASSERT_EQ (store.account.count (transaction), ledger.account_count ());
...
TEST (ledger, epoch_blocks_v1_general)
{
	...
	auto genesis_info = ledger.any.get (transaction, nano::dev::genesis_key.pub);
	...
	genesis_info = ledger.any.get (transaction, nano::dev::genesis_key.pub);
	ASSERT_TRUE (genesis_info);
	...
	ASSERT_EQ (0, ledger.any.balance (transaction, epoch4->hash ()));
	ASSERT_EQ (nano::Gxrb_ratio, ledger.any.balance (transaction, receive2->hash ()));
	ASSERT_EQ (nano::Gxrb_ratio, ledger.any.amount (transaction, receive2->hash ()));
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gxrb_ratio, ledger.weight (nano::dev::genesis_key.pub));
...
TEST (ledger, epoch_blocks_v2_general)
{
	...
	ASSERT_EQ (nano::epoch::epoch_1, receive2->sideband ().source_epoch);
	ASSERT_EQ (0, ledger.any.balance (transaction, epoch6->hash ()));
	ASSERT_EQ (nano::Gxrb_ratio, ledger.any.balance (transaction, receive2->hash ()));
	ASSERT_EQ (nano::Gxrb_ratio, ledger.any.amount (transaction, receive2->hash ()));
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gxrb_ratio, ledger.weight (nano::dev::genesis_key.pub));
	ASSERT_EQ (nano::Gxrb_ratio, ledger.weight (destination.pub));
...
TEST (ledger, epoch_blocks_receive_upgrade)
{
	...
	ASSERT_EQ (nano::epoch::epoch_1, receive2->sideband ().details.epoch);
	ASSERT_EQ (nano::epoch::epoch_1, receive2->sideband ().source_epoch);
	auto destination_info = ledger.any.get (transaction, destination.pub);
	...
	destination_info = ledger.any.get (transaction, destination.pub);
	ASSERT_TRUE (destination_info);
	ASSERT_EQ (destination_info->epoch (), nano::epoch::epoch_0);
	auto pending_send2 = ledger.any.get (transaction, nano::pending_key (destination.pub, send2->hash ()));
...
TEST (ledger, successor_epoch)
{
	...
	ASSERT_EQ (*change, *node1.ledger.any.get (transaction, node1.ledger.any.successor (transaction, change->qualified_root ()).value ()));
	ASSERT_EQ (*epoch_open, *node1.ledger.any.get (transaction, node1.ledger.any.successor (transaction, epoch_open->qualified_root ()).value ()));
...
TEST (ledger, unchecked_epoch)
{
	...
	ASSERT_TIMELY (5s, node1.ledger.any.exists (node1.store.tx_begin_read (), epoch1->hash ()));
	{
		// Waits for the last blocks to pass through block_processor and unchecked.put queues
		ASSERT_TIMELY_EQ (10s, 0, node1.unchecked.count ());
		auto info = node1.ledger.any.get (node1.store.tx_begin_read (), destination.pub);
		ASSERT_TRUE (info);
...
TEST (ledger, unchecked_epoch_invalid)
{
	...
	node1.block_processor.add (open1);
	// Waits for the last blocks to pass through block_processor and unchecked.put queues
	ASSERT_TIMELY (10s, node1.ledger.any.exists (node1.store.tx_begin_read (), epoch2->hash ()));
	{
		auto transaction = node1.store.tx_begin_read ();
		ASSERT_FALSE (node1.ledger.any.exists (transaction, epoch1->hash ()));
		auto unchecked_count = node1.unchecked.count ();
		ASSERT_EQ (unchecked_count, 0);
		ASSERT_EQ (unchecked_count, node1.unchecked.count ());
		auto info = node1.ledger.any.get (transaction, destination.pub);
		ASSERT_TRUE (info);
		ASSERT_NE (info->epoch (), nano::epoch::epoch_1);
		auto epoch2_store = node1.ledger.any.get (transaction, epoch2->hash ());
...
TEST (ledger, unchecked_open)
{
	...
	ASSERT_TIMELY (5s, node1.ledger.any.exists (node1.store.tx_begin_read (), open1->hash ()));
...
TEST (ledger, unchecked_receive)
{
	...
	ASSERT_TIMELY (10s, node1.ledger.any.exists (node1.store.tx_begin_read (), receive1->hash ()));
	ASSERT_EQ (0, node1.unchecked.count ());
}
...
TEST (ledger, confirmation_height_not_updated)
{
	...
	auto transaction = store.tx_begin_write ();
	auto & pool = ctx.pool ();
	auto account_info = ledger.any.get (transaction, nano::dev::genesis_key.pub);
...
TEST (ledger, dependents_confirmed_pruning)
{
	...
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, send2));
	ledger.confirm (transaction, send2->hash ());
	ASSERT_TRUE (ledger.confirmed.exists_or_pruned (transaction, send1->hash ()));
...
TEST (ledger, block_confirmed)
{
	...
	auto transaction = store.tx_begin_write ();
	nano::block_builder builder;
	ASSERT_TRUE (ledger.confirmed.exists_or_pruned (transaction, nano::dev::genesis->hash ()));
	auto & pool = ctx.pool ();
	nano::keypair key1;
	auto send1 = builder.state ()
				 .account (nano::dev::genesis_key.pub)
	...
	ASSERT_FALSE (ledger.confirmed.exists_or_pruned (transaction, send1->hash ()));
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, send1));
	ASSERT_FALSE (ledger.confirmed.exists_or_pruned (transaction, send1->hash ()));
	ledger.confirm (transaction, send1->hash ());
	ASSERT_TRUE (ledger.confirmed.exists_or_pruned (transaction, send1->hash ()));
...
TEST (ledger, cache)
{
	...
	for (size_t i (0); i < total; ++i)
	{
		...
		auto cache_check = [&, i] (nano::ledger const & ledger) {
			...
			ASSERT_EQ (genesis_weight, ledger.weight (nano::dev::genesis_key.pub));
			ASSERT_EQ (pruned_count, ledger.pruned_count ());
		};

		nano::keypair key;
		auto const latest = ledger.any.head (store.tx_begin_read (), nano::dev::genesis_key.pub);
...

		{
			auto transaction (store.tx_begin_write ());
			ledger.confirm (transaction, send->hash ());
			ASSERT_TRUE (ledger.confirmed.exists_or_pruned (transaction, send->hash ()));
		}
...

		{
			...
			ASSERT_TRUE (ledger.confirmed.exists_or_pruned (transaction, open->hash ()));
		}
...
TEST (ledger, pruning_action)
{
	...
	auto send2 = builder
				 ...
				 .build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, send2));
	ASSERT_TRUE (ledger.any.exists (transaction, send2->hash ()));
	// Pruning action
	ledger.confirm (transaction, send1->hash ());
	...
	ASSERT_TRUE (ledger.any.get (transaction, nano::pending_key{ nano::dev::genesis_key.pub, send1->hash () }));
	ASSERT_FALSE (ledger.any.exists (transaction, send1->hash ()));
	ASSERT_TRUE (ledger.any.exists_or_pruned (transaction, send1->hash ()));
	...
	ASSERT_TRUE (ledger.any.exists (transaction, nano::dev::genesis->hash ()));
	ASSERT_TRUE (ledger.any.exists (transaction, send2->hash ()));
...
TEST (ledger, pruning_large_chain)
{
	...
	for (auto i (0); i < send_receive_pairs; i++)
	{
		auto send = builder
					...
					.build ();
		ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, send));
		ASSERT_TRUE (ledger.any.exists (transaction, send->hash ()));
		auto receive = builder
					   .state ()
					   .account (nano::dev::genesis_key.pub)
					   .previous (send->hash ())
					   ...
					   .build ();
		ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, receive));
		ASSERT_TRUE (ledger.any.exists (transaction, receive->hash ()));
	...
	ASSERT_TRUE (ledger.any.exists (transaction, nano::dev::genesis->hash ()));
	ASSERT_FALSE (ledger.any.exists (transaction, last_hash));
	ASSERT_EQ (store->pruned.count (transaction), ledger.pruned_count ());
	ASSERT_EQ (store->block.count (transaction), ledger.block_count () - ledger.pruned_count ());
...
TEST (ledger, pruning_source_rollback)
{
	...
	ASSERT_FALSE (ledger.rollback (transaction, receive1->hash ()));
	auto info2 = ledger.any.get (transaction, nano::pending_key (nano::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info2);
...
TEST (ledger, pruning_source_rollback_legacy)
{
	...
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, send2));
	ASSERT_TRUE (ledger.any.exists (transaction, send2->hash ()));
	ASSERT_TRUE (ledger.any.get (transaction, nano::pending_key{ key1.pub, send2->hash () }));
	auto send3 = builder
				 .send ()
				 .previous (send2->hash ())
				 .destination (nano::dev::genesis_key.pub)
				 ...
				 .build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, send3));
	ASSERT_TRUE (ledger.any.exists (transaction, send3->hash ()));
	...
	ASSERT_TRUE (store->pruned.exists (transaction, send1->hash ()));
	ASSERT_TRUE (ledger.any.exists (transaction, nano::dev::genesis->hash ()));
	auto info1 = ledger.any.get (transaction, nano::pending_key (nano::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info1);
	...
	auto info2 = ledger.any.get (transaction, nano::pending_key (key1.pub, send2->hash ()));
	ASSERT_TRUE (info2);
	ASSERT_EQ (nano::dev::genesis_key.pub, info2->source);
	ASSERT_EQ (nano::Gxrb_ratio, info2->amount.number ());
	...
	// Rollback receive block
	ASSERT_FALSE (ledger.rollback (transaction, receive1->hash ()));
	auto info3 = ledger.any.get (transaction, nano::pending_key (nano::dev::genesis_key.pub, send1->hash ()));
	ASSERT_TRUE (info3);
	ASSERT_EQ (nano::dev::genesis_key.pub, info3->source);
	...
	auto open1 = builder
				 ...
				 .build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, open1));
	ASSERT_FALSE (ledger.any.get (transaction, nano::pending_key{ key1.pub, send2->hash () }));
	ASSERT_EQ (2, ledger.pruned_count ());
	ASSERT_EQ (6, ledger.block_count ());
	// Rollback open block
	ASSERT_FALSE (ledger.rollback (transaction, open1->hash ()));
	auto info4 = ledger.any.get (transaction, nano::pending_key (key1.pub, send2->hash ()));
	ASSERT_TRUE (info4);
	ASSERT_EQ (nano::dev::genesis_key.pub, info4->source);
...
TEST (ledger, pruning_legacy_blocks)
{
	...
	auto send3 = builder
				 ...
				 .build ();
	ASSERT_EQ (nano::block_status::progress, ledger.process (transaction, send3));
	ledger.confirm (transaction, send3->hash ());
	// Pruning action
...
TEST (ledger, pruning_safe_functions)
{
	...
	ASSERT_EQ (1, ledger.pruning_action (transaction, send1->hash (), 1));
	ASSERT_FALSE (ledger.any.exists (transaction, send1->hash ()));
	ASSERT_TRUE (ledger.any.exists_or_pruned (transaction, send1->hash ())); // true for pruned
	ASSERT_TRUE (store->pruned.exists (transaction, send1->hash ()));
	ASSERT_TRUE (ledger.any.exists (transaction, nano::dev::genesis->hash ()));
	ASSERT_TRUE (ledger.any.exists (transaction, send2->hash ()));
	// Safe ledger actions
	ASSERT_FALSE (ledger.any.balance (transaction, send1->hash ()));
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gxrb_ratio * 2, ledger.any.balance (transaction, send2->hash ()).value ());
	ASSERT_FALSE (ledger.any.amount (transaction, send2->hash ()));
	ASSERT_FALSE (ledger.any.account (transaction, send1->hash ()));
	ASSERT_EQ (nano::dev::genesis_key.pub, ledger.any.account (transaction, send2->hash ()).value ());
}
...
TEST (ledger, head_block)
{
	...
	auto & store = ctx.store ();
	auto tx = store.tx_begin_read ();
	ASSERT_EQ (*nano::dev::genesis, *ledger.any.get (tx, ledger.any.head (tx, nano::dev::genesis_key.pub)));
}
...
{
	auto ctx = nano::test::context::ledger_empty ();
	ASSERT_EQ (ctx.ledger ().any.receivable_end (), ctx.ledger ().any.receivable_upper_bound (ctx.store ().tx_begin_read (), 0));
}
