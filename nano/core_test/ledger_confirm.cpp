...
TEST (ledger_confirm, single)
{
	...
	ASSERT_FALSE (node->ledger.confirmed.exists (transaction, send1->hash ()));
	node->ledger.confirm (transaction, send1->hash ());
	ASSERT_TRUE (node->ledger.confirmed.exists (transaction, send1->hash ()));
...
TEST (ledger_confirm, multiple_accounts)
{
	...
	ASSERT_FALSE (node->ledger.rollback (transaction, node->ledger.any.head (transaction, key2.pub)));
	ASSERT_FALSE (node->ledger.rollback (transaction, node->ledger.any.head (transaction, key1.pub)));
	ASSERT_TRUE (node->ledger.rollback (transaction, node->ledger.any.head (transaction, key1.pub)));
	ASSERT_TRUE (node->ledger.rollback (transaction, node->ledger.any.head (transaction, key2.pub)));

	// Confirm the other latest can't be rolled back either
	ASSERT_TRUE (node->ledger.rollback (transaction, node->ledger.any.head (transaction, key3.pub)));
	ASSERT_TRUE (node->ledger.rollback (transaction, node->ledger.any.head (transaction, nano::dev::genesis_key.pub)));
