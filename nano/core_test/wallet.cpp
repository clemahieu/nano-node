...
TEST (wallet, spend_all_one)
{
	...
	auto info2 = node1.ledger.any.get (transaction, nano::dev::genesis_key.pub);
	ASSERT_NE (latest1, info2->head);
	auto block = node1.ledger.any.get (transaction, info2->head);
	ASSERT_NE (nullptr, block);
...
TEST (wallet, spend_no_previous)
{
	nano::test::system system (1);
	{
		...
		auto info1 = system.nodes[0]->ledger.any.get (transaction, nano::dev::genesis_key.pub);
		ASSERT_TRUE (info1);
		for (auto i (0); i < 50; ++i)
		{
			nano::keypair key;
...
TEST (wallet, work_generate)
{
	...
	auto transaction (node1.store.tx_begin_read ());
	ASSERT_TIMELY (10s, node1.ledger.any.balance (transaction, nano::dev::genesis_key.pub) != amount1);
...
TEST (wallet, search_receivable)
{
	...
	auto receive_hash = node.ledger.any.head (node.store.tx_begin_read (), nano::dev::genesis_key.pub);
...
TEST (wallet, receive_pruned)
{
	...
	ASSERT_EQ (amount, node2.ledger.any.balance (node2.store.tx_begin_read (), open1->hash ()));
