	...
	if (ec == nano::error_cli::unknown_command)
	{
		...
		else if (vm.count ("validate_blocks") || vm.count ("debug_validate_blocks"))
		{
			...
			auto check_account = [&print_error_message, &silent, &count, &block_count] (std::shared_ptr<nano::node> const & node, nano::store::read_transaction const & transaction, nano::account const & account, nano::account_info const & info) {
				...
				auto block = node->ledger.any.get (transaction, hash); // Block data
				uint64_t height (0);
				if (node->ledger.pruning && confirmation_height_info.height != 0)
				{
					...
					{
						auto previous_block = node->ledger.any.get (transaction, block->previous ());
					...
					if (!node->ledger.any.exists_or_pruned (transaction, info.open_block))
					{
				...
				while (!hash.is_zero () && block != nullptr)
				{
					...
					if (validate_message (account, hash, block->block_signature ()))
					{
						...
						if (block->type () == nano::block_type::state)
						{
							...
							{
								prev_balance = node->ledger.any.balance (transaction, state_block.hashables.previous).value_or (0);
							}
					...
					{
						auto prev_balance = node->ledger.any.balance (transaction, block->previous ());
						if (!node->ledger.pruning || prev_balance)
						{
							...
							else
							{
								...
								else
								{
									...
									block_details_error |= !node->ledger.any.exists_or_pruned (transaction, block->source ());
								}
							}
						}
			...
			auto check_pending = [&print_error_message, &silent, &count] (std::shared_ptr<nano::node> const & node, nano::store::read_transaction const & transaction, nano::pending_key const & key, nano::pending_info const & info) {
				...
				auto block = node->ledger.any.get (transaction, key.hash);
				bool pruned (false);
				if (block == nullptr)
				{
				...
				else
				{
					...
					if (previous_pruned)
					{
						block = node->ledger.any.get (transaction, key.hash);
					}
					...
					auto account (node->ledger.any.account (transaction, key.hash));
					...
					{
						auto amount (node->ledger.any.amount (transaction, key.hash));
		...
		else if (vm.count ("debug_profile_bootstrap"))
		{
			...
			std::deque<std::shared_ptr<nano::block>> epoch_open_blocks;
			{
				...
				for (auto i (source_node->store.account.begin (transaction)), n (source_node->store.account.end ()); i != n; ++i)
				{
					...
					{
						// Retrieving block data
						auto block = source_node->ledger.any.get (transaction, hash);
						if (block != nullptr)
						{
							++count;
							if ((count % 500000) == 0)
