...
void nano::bulk_pull_server::set_current_end ()
{
	...
	if (!node->ledger.any.exists (transaction, request->end))
	{
	...
	if (node->ledger.any.exists (transaction, request->start.as_block_hash ()))
	{
		node->logger.debug (nano::log::type::bulk_pull_server, "Bulk pull request for block hash: {}", request->start.to_string ());

		current = ascending () ? node->ledger.any.successor (transaction, request->start.as_block_hash ()).value_or (0) : request->start.as_block_hash ();
		include_start = true;
	}
	else
	{
		auto info = node->ledger.any.get (transaction, request->start.as_account ());
		if (!info)
		{
		...
		else
		{
			...
			{
				auto account (node->ledger.any.account (transaction, request->end));
...
void nano::bulk_pull_account_server::send_frontier ()
{
	...
	if (!invalid_request)
	{
		...
		auto account_frontier_hash (node->ledger.any.head (stream_transaction, request->account));
		auto account_frontier_balance_int = node->ledger.any.balance (stream_transaction, request->account).value_or (0);
