...
void nano::epoch_upgrader::upgrade_impl (nano::raw_key const & prv_a, nano::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	...
	while (!finished_upgrade && !stopped)
	{
		...
		while (!finished_pending && count_limit != 0 && !stopped)
		{
			...
			uint64_t attempts (0);
			auto transaction = store.tx_begin_read ();
			for (auto current = ledger.any.receivable_upper_bound (transaction, 0), end = ledger.any.receivable_end (); current != end && attempts < upgrade_batch_size && attempts < count_limit && !stopped;)
			{
				...
				if (!store.account.exists (transaction, key.account))
				{
					...
					current = ledger.any.receivable_upper_bound (transaction, key.account, key.hash);
				}
				else
				{
					...
					{
						current = ledger.any.receivable_upper_bound (transaction, key.account);
					}
				}
			}
			{
