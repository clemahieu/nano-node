...
nano::account_info nano::json_handler::account_info_impl (store::transaction const & transaction_a, nano::account const & account_a)
{
	...
	if (!ec)
	{
		auto info = node.ledger.any.get (transaction_a, account_a);
		if (!info)
		{
...
uint64_t nano::json_handler::difficulty_ledger (nano::block const & block_a)
{
	...
	if (!previous.is_zero ())
	{
		block_previous = node.ledger.any.get (transaction, previous);
	}
	// Send check
	if (block_previous != nullptr)
	{
		details.is_send = node.ledger.any.balance (transaction, previous) > block_a.balance_field ().value ().number ();
		details_found = true;
	}
	// Epoch check
	if (block_previous != nullptr)
	...
	{
		auto block_link = node.ledger.any.get (transaction, link.value ().as_block_hash ());
		auto account = block_a.account_field ().value (); // Link is non-zero therefore it's a state block and has an account field;
		if (block_link != nullptr && node.ledger.any.get (transaction, nano::pending_key{ account, link.value ().as_block_hash () }))
		{
	...
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			if (include_confirmed)
			{
				...
				{
					confirmed_balance_l = node.ledger.any.balance (transaction, confirmation_height_info.frontier).value_or (0);
				}
				else
				{
			...
			{
				confirmed_frontier_block = node.ledger.any.get (transaction, confirmation_height_info.frontier);
			}
			...
			if (representative)
			{
				...
				if (include_confirmed)
				{
					...
					if (confirmed_frontier_block)
					{
						...
						{
							confirmed_representative = node.ledger.any.get (transaction, node.ledger.representative (transaction, confirmation_height_info.frontier))->representative_field ().value ();
						}
					}
...
void nano::json_handler::accounts_frontiers ()
{
	...
	for (auto & account_from_request : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			auto latest = node.ledger.any.head (transaction, account);
			if (!latest.is_zero ())
			{
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			...
			for (auto current = node.ledger.any.receivable_upper_bound (transaction, account, 0), end = node.ledger.any.receivable_end (); current != end && peers_l.size () < count; ++current)
			{
				...
				if (include_only_confirmed && !node.ledger.confirmed.exists_or_pruned (transaction, key.hash))
				{
					continue;
				}
				if (simple)
...
void nano::json_handler::block_info ()
{
	...
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			...
			auto amount = node.ledger.any.amount (transaction, hash);
			if (amount)
			{
			...
			auto confirmed (node.ledger.confirmed.exists_or_pruned (transaction, hash));
...
void nano::json_handler::block_confirm ()
{
	...
	{
		auto transaction (node.store.tx_begin_read ());
		auto block_l = node.ledger.any.get (transaction, hash);
		if (block_l != nullptr)
		{
			if (!node.ledger.confirmed.exists_or_pruned (transaction, hash))
			{
			...
			if (!hash.decode_hex (hash_text))
			{
				...
				if (block != nullptr)
				{
					...
					if (receivable || receive_hash)
					{
						...
						else if (node.ledger.any.get (transaction, nano::pending_key{ block->destination (), hash }))
						{
							if (receivable)
							{
								entry.put ("pending", "1");
					...
					if (source)
					{
						if (!block->is_receive () || !node.ledger.any.exists (transaction, block->source ()))
						{
							entry.put ("source_account", "0");
						}
						else
						{
							auto block_a = node.ledger.any.get (transaction, block->source ());
...
void nano::json_handler::block_create ()
{
	...
	if (!ec)
	{
		...
		if (prv != 0)
		{
			...
			{
				auto transaction = node.store.tx_begin_read ();
				if (node.ledger.any.exists (transaction, previous) && node.ledger.any.balance (transaction, previous) != balance.number ())
				{
	...
	if (!ec)
	{
		...
		while (!hash.is_zero () && blocks.size () < count)
		{
			...
			if (block_l != nullptr)
			{
				...
				hash = successors ? node.ledger.any.successor (transaction, hash).value_or (0) : block_l->previous ();
			}
			else
			{
				hash.clear ();
...
public:
	...
	void send_block (nano::send_block const & block_a)
	{
		...
		auto amount = handler.node.ledger.any.amount (transaction, hash);
		if (amount)
		{
	...
	void receive_block (nano::receive_block const & block_a)
	{
		...
		if (amount)
		{
			auto source_account = handler.node.ledger.any.account (transaction, block_a.hashables.source);
			if (source_account)
			{
	...
	void state_block (nano::state_block const & block_a)
	{
		...
		auto previous_balance = handler.node.ledger.any.balance (transaction, block_a.hashables.previous);
		if (!previous_balance)
		{
			if (raw)
			{
		...
		else
		{
			...
			{
				auto source_account = handler.node.ledger.any.account (transaction, block_a.hashables.link.as_block_hash ());
...
void nano::json_handler::account_history ()
{
	...
	if (head_str)
	{
		...
		{
			if (node.ledger.any.exists (transaction, hash))
			{
				account = node.ledger.any.account (transaction, hash).value ();
			}
			else
			{
				ec = nano::error_blocks::not_found;
	...
	else
	{
		...
		if (!ec)
		{
			...
			{
				hash = node.ledger.any.head (transaction, account);
			}
		}
	}
	if (!ec)
		...
		while (block != nullptr && count > 0)
		{
			...
			else
			{
				...
				if (!entry.empty ())
				{
					...
					entry.put ("confirmed", node.ledger.confirmed.exists_or_pruned (transaction, hash));
					if (output_raw)
					{
			...
			hash = reverse ? node.ledger.any.successor (transaction, hash).value_or (0) : block->previous ();
			block = node.ledger.any.get (transaction, hash);
		}
...
void nano::json_handler::receivable_exists ()
{
	...
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			...
			if (block->is_send ())
			{
				exists = node.ledger.any.get (transaction, nano::pending_key{ block->destination (), hash }).has_value ();
			}
	...
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<nano::json_handler> const & rpc_l) {
		...
		if (!rpc_l->ec && block->type () == nano::block_type::state)
		{
			...
			if (!subtype_text.empty ())
			{
				...
				if (!block_state->hashables.previous.is_zero () && !rpc_l->node.ledger.any.exists (transaction, block_state->hashables.previous))
				{
...
void nano::json_handler::receive ()
{
	...
	if (!ec)
	{
		...
		{
			auto block_transaction = node.store.tx_begin_read ();
			if (node.ledger.any.exists_or_pruned (block_transaction, hash))
			{
				auto pending_info = node.ledger.any.get (block_transaction, nano::pending_key (account, hash));
				if (pending_info)
				{
					auto work (work_optional_impl ());
					if (!ec && work)
					{
						...
						auto info = node.ledger.any.get (block_transaction, account);
						if (info)
						{
							head = info->head;
		...
		if (block != nullptr)
		{
			...
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				...
				if (sources != 0) // Republish source chain
				{
					...
					auto block_a = node.ledger.any.get (transaction, source);
					std::vector<nano::block_hash> hashes;
					while (block_a != nullptr && hashes.size () < sources)
					{
						hashes.push_back (source);
						source = block_a->previous ();
						block_a = node.ledger.any.get (transaction, source);
					}
					std::reverse (hashes.begin (), hashes.end ());
					for (auto & hash_l : hashes)
					{
						block_a = node.ledger.any.get (transaction, hash_l);
				...
				{
					auto block_b = node.ledger.any.get (transaction, hash);
					auto destination = block_b->destination ();
					if (!destination.is_zero ())
					{
						if (!node.ledger.any.get (transaction, nano::pending_key{ destination, hash }))
						{
							nano::block_hash previous (node.ledger.any.head (transaction, destination));
							auto block_d = node.ledger.any.get (transaction, previous);
							...
							while (block_d != nullptr && hash != source)
							{
								...
								block_d = node.ledger.any.get (transaction, previous);
							}
							std::reverse (hashes.begin (), hashes.end ());
							...
							for (auto & hash_l : hashes)
							{
								block_d = node.ledger.any.get (transaction, hash_l);
				...
				hash = node.ledger.any.successor (transaction, hash).value_or (0);
			}
...
void nano::json_handler::wallet_info ()
{
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			auto account_info = node.ledger.any.get (block_transaction, account);
			if (account_info)
			{
...
void nano::json_handler::wallet_frontiers ()
{
	...
	if (!ec)
	{
		...
		{
			nano::account const & account (i->first);
			auto latest (node.ledger.any.head (block_transaction, account));
			if (!latest.is_zero ())
			{
				frontiers.put (account.to_account (), latest.to_string ());
			}
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			if (info)
			{
				...
				{
					auto block = node.ledger.any.get (block_transaction, hash);
...
void nano::json_handler::wallet_receivable ()
{
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			{
				auto const & [key, info] = *current;
				if (include_only_confirmed && !node.ledger.confirmed.exists_or_pruned (block_transaction, key.hash))
				{
					continue;
				}
		...
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && update_existing_accounts)
			{
				std::vector<nano::account> accounts;
				{
					...
					{
						nano::account const & account (i->first);
						auto info = rpc_l->node.ledger.any.get (block_transaction, account);
						if (info)
						{
							if (info->representative != representative)
							{
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			while (!latest.is_zero () && hashes.size () < count)
			{
				...
				block = node.ledger.any.get (block_transaction, latest);
				if (block != nullptr)
				{
			...
			{
				block = node.ledger.any.get (block_transaction, hash);
		...
		if (!ec && response_l.empty ())
		{
			...
			else
			{
				if (!account_opt.is_initialized ())
				{
					...
					if (node.ledger.any.exists (transaction_l, hash))
					{
						account = node.ledger.any.account (transaction_l, hash).value ();
					}
				}
...
bool block_confirmed (nano::node & node, nano::store::transaction & transaction, nano::block_hash const & hash, bool include_active, bool include_only_confirmed)
{
	...
	else if (node.ledger.confirmed.exists_or_pruned (transaction, hash))
	{
		is_confirmed = true;
	}
