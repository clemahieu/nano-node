...
{
	auto const transaction (store.tx_begin_read ());
	return ledger.any.head (transaction, account_a);
}

nano::uint128_t nano::node::balance (nano::account const & account_a)
{
	auto const transaction (store.tx_begin_read ());
	return ledger.any.balance (transaction, account_a).value_or (0);
}

std::shared_ptr<nano::block> nano::node::block (nano::block_hash const & hash_a)
{
	return ledger.any.get (store.tx_begin_read (), hash_a);
}

bool nano::node::block_or_pruned_exists (nano::block_hash const & hash_a) const
{
	return ledger.any.exists_or_pruned (store.tx_begin_read (), hash_a);
}
...
std::pair<nano::uint128_t, nano::uint128_t> nano::node::balance_pending (nano::account const & account_a, bool only_confirmed_a)
{
	...
	auto const transaction (store.tx_begin_read ());
	result.first = only_confirmed_a ? ledger.confirmed.balance (transaction, account_a).value_or (0) : ledger.any.balance (transaction, account_a).value_or (0);
...
{
	auto transaction (store.tx_begin_read ());
	return ledger.confirmed.exists_or_pruned (transaction, hash_a);
}

bool nano::node::block_confirmed_or_being_confirmed (nano::store::transaction const & transaction, nano::block_hash const & hash_a)
{
	return confirming_set.exists (hash_a) || ledger.confirmed.exists_or_pruned (transaction, hash_a);
}
...
void nano::node::receive_confirmed (store::transaction const & block_transaction_a, nano::block_hash const & hash_a, nano::account const & destination_a)
{
	...
	for ([[maybe_unused]] auto const & [id, wallet] : wallets_l)
	{
		if (wallet->store.exists (wallet_transaction, destination_a))
		{
			...
			auto pending = ledger.any.get (block_transaction_a, nano::pending_key (destination_a, hash_a));
			if (pending)
			{
			...
			{
				if (!ledger.any.exists_or_pruned (block_transaction_a, hash_a))
				{
