	...
	for (auto const & [hash, root] : requests_a)
	{
		...
		else
		{
			...
			if (!final_vote_hashes.empty ())
			{
				...
				block = ledger.any.get (transaction, final_vote_hashes[0]);
				// Allow same root vote
				if (block != nullptr && final_vote_hashes.size () > 1)
				{
					to_generate_final.push_back (block);
					block = ledger.any.get (transaction, final_vote_hashes[1]);
			...
			if (block == nullptr)
			{
				block = ledger.any.get (transaction, hash);
			...
			if (block == nullptr && !root.is_zero ())
			{
				...
				auto successor = ledger.any.successor (transaction, root.as_block_hash ());
				if (successor)
				{
					auto successor_block = ledger.any.get (transaction, successor.value ());
