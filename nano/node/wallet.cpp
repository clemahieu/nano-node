...
void nano::wallet_store::serialize_json (store::transaction const & transaction_a, std::string & string_a)
{
	...
	for (iterator i{ store::iterator{ store::lmdb::iterator{ env.tx (transaction_a), handle } } }, n{ store::iterator{ store::lmdb::iterator{} } }; i != n; ++i)
	{
	...
	stopped (false)
{
	...
	if (!error_a)
	{
		...
		store::typed_iterator<std::array<char, 64>, nano::no_value> i{ store::iterator{ store::lmdb::iterator{ env.tx (transaction), handle, beginning_val } } };
		store::typed_iterator<std::array<char, 64>, nano::no_value> n{ store::iterator{ store::lmdb::iterator{ env.tx (transaction), handle, end_val } } };
		for (; i != n; ++i)
		{
			nano::wallet_id id;
...
{
	nano::store::lmdb::db_val val{ nano::account (special_count) };
	return iterator{ store::iterator{ store::lmdb::iterator{ env.tx (transaction_a), handle, val } } };
}

auto nano::wallet_store::begin (store::transaction const & transaction_a, nano::account const & key) -> iterator
{
	nano::store::lmdb::db_val val{ nano::account (key) };
	return iterator{ store::iterator{ store::lmdb::iterator{ env.tx (transaction_a), handle, val } } };
}
...
{
	return iterator{ store::iterator{ store::lmdb::iterator{} } };
}
