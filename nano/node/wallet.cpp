...
std::shared_ptr<nano::block> nano::wallet::receive_action (nano::block_hash const & send_hash_a, nano::account const & representative_a, nano::uint128_union const & amount_a, nano::account const & account_a, uint64_t work_a, bool generate_work_a)
{
	...
	if (wallets.node.config.receive_minimum.number () <= amount_a.number ())
	{
		...
		auto transaction (wallets.tx_begin_read ());
		if (wallets.node.ledger.any.exists_or_pruned (block_transaction, send_hash_a))
		{
			auto pending_info = wallets.node.ledger.any.get (block_transaction, nano::pending_key (account_a, send_hash_a));
			if (pending_info)
			{
				nano::raw_key prv;
				if (!store.fetch (transaction, account_a, prv))
				{
					...
					auto info = wallets.node.ledger.any.get (block_transaction, account_a);
					if (info)
					{
...
std::shared_ptr<nano::block> nano::wallet::change_action (nano::account const & source_a, nano::account const & representative_a, uint64_t work_a, bool generate_work_a)
{
	...
	nano::block_details details;
	{
		...
		if (store.valid_password (transaction))
		{
			...
			if (existing != store.end () && !wallets.node.ledger.any.head (block_transaction, source_a).is_zero ())
			{
				auto info = wallets.node.ledger.any.get (block_transaction, source_a);
				debug_assert (info);
				nano::raw_key prv;
	...
	auto prepare_send = [&id_mdb_val, &wallets = this->wallets, &store = this->store, &source_a, &amount_a, &work_a, &account_a] (auto const & transaction) {
		...
		if (id_mdb_val)
		{
			...
			if (status == 0)
			{
				...
				block = wallets.node.ledger.any.get (block_transaction, hash);
				if (block != nullptr)
				{
					cached_block = true;
		...
		if (!error && block == nullptr)
		{
			if (store.valid_password (transaction))
			{
				...
				{
					auto balance (wallets.node.ledger.any.balance (block_transaction, source_a));
					if (balance && balance.value () >= amount_a)
					{
						auto info = wallets.node.ledger.any.get (block_transaction, source_a);
						debug_assert (info);
						nano::raw_key prv;
						...
						block = std::make_shared<nano::state_block> (source_a, info->head, info->representative, balance.value () - amount_a, account_a, prv, source_a, work_a);
						details.epoch = info->epoch ();
			...
			if (!nano::wallet_value (i->second).key.is_zero ())
			{
				for (auto i = wallets.node.ledger.receivable_upper_bound (block_transaction, account, 0), n = wallets.node.ledger.receivable_end (); i != n; ++i)
				{
					...
					if (wallets.node.config.receive_minimum.number () <= amount)
					{
						...
						if (wallets.node.ledger.confirmed.exists_or_pruned (block_transaction, hash))
						{
						...
						{
							auto block = wallets.node.ledger.any.get (block_transaction, hash);
							if (block)
							{
...
uint32_t nano::wallet::deterministic_check (store::transaction const & transaction_a, uint32_t index)
{
	...
	for (uint32_t i (index + 1), n (index + 64); i < n; ++i)
	{
		...
		auto latest (wallets.node.ledger.any.head (block_transaction, pair.pub));
		if (!latest.is_zero ())
		{
			index = i;
