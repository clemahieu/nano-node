...
void nano_qt::accounts::refresh_wallet_balance ()
{
	...
	for (auto i (this->wallet.wallet_m->store.begin (transaction)), j (this->wallet.wallet_m->store.end ()); i != j; ++i)
	{
		...
		balance = balance + this->wallet.node.ledger.any.balance (block_transaction, key).value_or (0);
...
void nano_qt::accounts::refresh ()
{
	...
	for (auto i (wallet.wallet_m->store.begin (transaction)), j (wallet.wallet_m->store.end ()); i != j; ++i)
	{
		...
		auto balance_amount = wallet.node.ledger.any.balance (block_transaction, key).value_or (0);
		bool display (true);
...
public:
	...
	void send_block (nano::send_block const & block_a)
	{
		...
		auto amount_l = ledger.any.amount (transaction, block_a.hash ());
		if (!amount_l)
		{
			type = "Send (pruned)";
		}
	...
	{
		type = "Receive";
		auto account_l = ledger.any.account (transaction, block_a.hashables.source);
		auto amount_l = ledger.any.amount (transaction, block_a.hash ());
	...
	void state_block (nano::state_block const & block_a)
	{
		...
		auto previous_balance = ledger.any.balance (transaction, block_a.hashables.previous);
		if (!previous_balance)
		{
			type = "Unknown (pruned)";
			amount = 0;
		...
		else
		{
			...
			{
				type = "Receive";
				auto account_l = ledger.any.account (transaction, block_a.hashables.link.as_block_hash ());
				if (!account_l)
				{
					type = "Receive (pruned)";
				}
	...
	wallet (wallet_a)
{
	...
	QObject::connect (retrieve, &QPushButton::released, [this] () {
		...
		if (!hash_l.decode_hex (hash->text ().toStdString ()))
		{
			...
			auto block_l (this->wallet.node.ledger.any.get (transaction, hash_l));
			if (block_l != nullptr)
			{
				std::string contents;
				block_l->serialize_json (contents);
				block->setPlainText (contents.c_str ());
				auto successor_l = this->wallet.node.ledger.any.successor (transaction, hash_l).value_or (0);
	...
	QObject::connect (rebroadcast, &QPushButton::released, [this] () {
		...
		{
			auto transaction = this->wallet.node.store.tx_begin_read ();
			if (this->wallet.node.ledger.any.exists (transaction, block))
			{
...
void nano_qt::block_viewer::rebroadcast_action (nano::block_hash const & hash_a)
{
	...
	auto block (wallet.node.ledger.any.get (transaction, hash_a));
	if (block != nullptr)
	{
		wallet.node.network.flood_block (block);
		auto successor = wallet.node.ledger.any.successor (transaction, hash_a);
		if (successor)
		{
			done = false;
	...
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				{
					auto balance = wallet.node.ledger.any.balance (block_transaction, account_l).value_or (0);
...
void nano_qt::block_creation::create_receive ()
{
	...
	if (!error)
	{
		auto transaction (wallet.node.wallets.tx_begin_read ());
		auto block_transaction (wallet.node.store.tx_begin_read ());
		auto block_l (wallet.node.ledger.any.get (block_transaction, source_l));
		if (block_l != nullptr)
		{
			...
			if (!destination.is_zero ())
			{
				...
				if (auto pending = wallet.node.ledger.any.get (block_transaction, pending_key))
				{
					nano::account_info info;
