...
TEST (wallet, DISABLED_process_block)
{
	...
	QTest::mouseClick (wallet->block_entry.process, Qt::LeftButton);
	{
		...
		system.deadline_set (10s);
		while (system.nodes[0]->ledger.any.exists (transaction, send.hash ()))
		{
			ASSERT_NO_ERROR (system.poll ());
		}
	}
...
TEST (wallet, republish)
{
	...
	nano::block_hash hash;
	{
		...
		auto latest (system.nodes[0]->ledger.any.head (transaction, nano::dev::genesis_key.pub));
...
TEST (wallet, DISABLED_synchronizing)
{
	...
	wallet->start ();
	{
		...
		auto latest (system1.nodes[0]->ledger.any.head (transaction, nano::dev::genesis_key.pub));
