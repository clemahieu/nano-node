...
TEST (rpc, wallet_representative_set_force)
{
	...
	while (representative != key.pub)
	{
		...
		auto info = node->ledger.any.get (transaction, nano::dev::genesis_key.pub);
		if (info)
		{
...
TEST (rpc, account_representative_set)
{
	...
	ASSERT_TIMELY (5s, node->ledger.confirmed.exists_or_pruned (node->store.tx_begin_read (), key2_open_block_hash));
	auto key2_open_block = node->ledger.any.get (node->store.tx_begin_read (), key2_open_block_hash);
	...
	auto block = node->ledger.any.get (node->store.tx_begin_read (), hash);
	ASSERT_NE (block, nullptr);
	ASSERT_TIMELY (5s, node->ledger.confirmed.exists_or_pruned (node->store.tx_begin_read (), hash));
...
TEST (rpc, account_representative_set_epoch_2_insufficient_work)
{
	...
	// Test that the correct error is given if there is insufficient work
	auto latest = node->ledger.any.head (node->store.tx_begin_read (), nano::dev::genesis_key.pub);
...
TEST (rpc, pending_exists)
{
	...
	node->ledger.any.get (node->store.tx_begin_read (), nano::pending_key{ nano::dev::genesis_key.pub, block1->hash () });
...
TEST (rpc, blocks_info_receive_hash)
{
	...
	ASSERT_EQ (node->ledger.confirmed.balance (node->store.tx_begin_read (), key1.pub), 10);
...
TEST (rpc, block_info_pruning)
{
	...
	// Pruning action
	{
		auto transaction = node1->store.tx_begin_write ();
		ASSERT_EQ (1, node1->ledger.pruning_action (transaction, send1->hash (), 1));
		ASSERT_TRUE (node1->ledger.any.exists (transaction, receive1->hash ()));
	}
	auto const rpc_ctx = add_rpc (system, node1);
	// Pruned block
	boost::property_tree::ptree request;
...
TEST (rpc, block_confirm_confirmed)
{
	...
	{
		auto transaction (node->store.tx_begin_read ());
		ASSERT_TRUE (node->ledger.confirmed.exists_or_pruned (transaction, nano::dev::genesis->hash ()));
	}
...
TEST (rpc, block_confirmed)
{
	...
	ASSERT_TIMELY (5s, node->ledger.confirmed.exists_or_pruned (node->store.tx_begin_read (), send->hash ()) && !node->confirming_set.exists (send->hash ()));
...
TEST (rpc, epoch_upgrade)
{
	...
	ASSERT_EQ (nano::block_status::progress, node->process (send7));
	{
		// Check pending entry
		auto transaction (node->store.tx_begin_read ());
		auto info = node->ledger.any.get (transaction, nano::pending_key (key3.pub, send7->hash ()));
		ASSERT_TRUE (info);
...
TEST (rpc, receive)
{
	...
	request.put ("block", send2->hash ().to_string ());
	{
		...
		auto info = node->ledger.any.get (node->store.tx_begin_read (), key1.pub);
		ASSERT_TRUE (info);
...
TEST (rpc, receive_unopened)
{
	...
	ASSERT_TRUE (node->ledger.any.exists (node->store.tx_begin_read (), send1->hash ()));
	...
	ASSERT_TRUE (node->ledger.any.exists (node->store.tx_begin_read (), send2->hash ()));
	nano::public_key rep;
	...
	request.put ("block", send2->hash ().to_string ());
	{
		auto response (wait_response (system, rpc_ctx, request));
		auto receive_text (response.get<std::string> ("block"));
		auto info = node->ledger.any.get (node->store.tx_begin_read (), key2.pub);
		ASSERT_TRUE (info);
		ASSERT_EQ (info->head, info->open_block);
		ASSERT_EQ (info->head.to_string (), receive_text);
...
TEST (rpc, receive_pruned)
{
	...
	ASSERT_FALSE (node2->ledger.any.exists (node2->store.tx_begin_read (), send1->hash ()));
	ASSERT_TRUE (node2->block_or_pruned_exists (send2->hash ()));
	ASSERT_FALSE (node2->ledger.any.exists (node2->store.tx_begin_read (), send2->hash ()));
	...
	request.put ("block", send2->hash ().to_string ());
	{
		auto response (wait_response (system, rpc_ctx, request));
		auto receive_text (response.get<std::string> ("block"));
		auto info = node2->ledger.any.get (node2->store.tx_begin_read (), key1.pub);
		ASSERT_TRUE (info);
		ASSERT_EQ (info->head, nano::block_hash{ receive_text });
	}
	// Trying to receive the same block should fail with unreceivable
