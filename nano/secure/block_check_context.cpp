...
void nano::block_check_context::load (store::transaction const & transaction)
{
	auto hash = block_m->hash ();
	if (ledger.any.exists_or_pruned (transaction, hash))
	{
	...
	{
		previous = ledger.any.get (transaction, block.previous ());
	}
	if (!gap_previous ())
	{
		auto account_l = account ();
		auto source_l = source ();
		state = ledger.any.get (transaction, account_l);
		if (!state)
		{
			state = nano::account_info{};
		}
		source_exists = ledger.any.exists_or_pruned (transaction, source_l);
		nano::pending_key key{ account_l, source_l };
		receivable = ledger.any.get (transaction, key);
		any_receivable = ledger.any.receivable_any (transaction, account_l);
		details = block_details{ epoch (), is_send (), is_receive (), is_epoch () };
	}
}

nano::block_status nano::block_check_context::check (store::transaction const & transaction)
{
	load (transaction);
	if (old ())
	{
