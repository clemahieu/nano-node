...
{
	auto snapshot = confirming.snapshot ();
	auto tx = store.tx_begin_write (tables_to_lock, tables_no_lock);
	return secure::write_transaction{ std::move (snapshot), std::move (tx) };
}
...
auto nano::ledger::tx_begin_read () const -> secure::read_transaction
{
	...
	return secure::read_transaction{ std::move (snapshot), std::move (tx) };
}
