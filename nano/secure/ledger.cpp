...
bool ledger_processor::validate_epoch_block (nano::state_block const & block_a)
{
	...
	if (!block_a.hashables.previous.is_zero ())
	{
		...
		{
			prev_balance = ledger.any.balance (transaction, block_a.hashables.previous).value ();
		}
		else
		{
...
{
	auto hash (block_a.hash ());
	auto existing = ledger.any.exists_or_pruned (transaction, hash);
			...
			if (result == nano::block_status::progress)
			{
				...
				if (result == nano::block_status::progress)
				{
					if (!is_send)
					{
						...
						{
							result = ledger.any.exists_or_pruned (transaction, block_a.hashables.link.as_block_hash ()) ? nano::block_status::progress : nano::block_status::gap_source; // Have we seen the source block already? (Harmless)
							if (result == nano::block_status::progress)
							{
		...
		if (result == nano::block_status::progress)
		{
			...
			if (result == nano::block_status::progress)
			{
				...
				if (result == nano::block_status::progress)
				{
					...
					{
						debug_assert (!validate_message (account, hash, block_a.signature));
						result = ledger.any.exists_or_pruned (transaction, block_a.hashables.source) ? nano::block_status::progress : nano::block_status::gap_source; // Have we seen the source block already? (Harmless)
						if (result == nano::block_status::progress)
						{
...
void ledger_processor::open_block (nano::open_block & block_a)
{
	...
	if (result == nano::block_status::progress)
	{
		...
		{
			debug_assert (!validate_message (block_a.hashables.account, hash, block_a.signature));
			result = ledger.any.exists_or_pruned (transaction, block_a.hashables.source) ? nano::block_status::progress : nano::block_status::gap_source; // Have we seen the source block? (Harmless)
			if (result == nano::block_status::progress)
			{
				nano::account_info info;
...
nano::uint128_t nano::ledger::account_balance (store::transaction const & transaction_a, nano::account const & account_a, bool only_confirmed_a)
{
	...
	if (only_confirmed_a)
	{
		...
		{
			result = any.balance (transaction_a, info.frontier).value ();
		}
	}
	else
	{
...
{
	auto block_l = any.get (transaction_a, hash_a);
	if (!block_l)
	{
		return std::nullopt;
	}
	...
	auto previous_balance = any.balance (transaction_a, block_l->previous ());
	if (!previous_balance)
	{
		return std::nullopt;
	}
...
std::optional<nano::block_hash> nano::ledger::successor (store::transaction const & transaction_a, nano::qualified_root const & root_a) const noexcept
{
	...
	{
		auto info = any.get (transaction_a, root_a.root ().as_account ());
		if (info)
		{
