...
TEST (flamegraph, large_confirmation_parallel)
{
	auto start = std::chrono::steady_clock::now ();
	auto circulating = 10 * nano::Gxrb_ratio;
	auto rep_count = 8;
	auto block_count = 8000;
	auto rep_amount = (nano::dev::constants.genesis_amount - circulating) / rep_count;
	std::deque<nano::keypair> reps;
	while (reps.size () < rep_count)
	{
		nano::keypair key{ nano::raw_key{ 1000 + reps.size () } }; // Hard code known keys
		reps.push_back (key);
	}
	std::filesystem::path snapshot = "ledger_snapshot";
	if (!std::filesystem::exists (snapshot))
	{
		std::filesystem::path path;
		{
			...
			for (auto rep : reps)
			{
				nano::test::setup_new_account (ctx.pool (), ctx.ledger (), rep_amount, nano::dev::genesis_key, rep, rep.pub, true);
			}

			auto tx = ctx.store ().tx_begin_write ();
			nano::state_block_builder builder;
			auto previous = ctx.store ().block.get (tx, ctx.ledger ().latest (tx, nano::dev::genesis_key.pub));
			for (auto i = 0; i < block_count; ++i)
			{
				nano::keypair key;
				auto block = builder.make_block ()
							 ...
							 .previous (previous->hash ())
							 .link (key.pub)
							 .balance (previous->balance ().number () - nano::xrb_ratio)
							 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
							 .work (*ctx.pool ().generate (previous->hash ()))
							 .build_shared ();
				ASSERT_EQ (nano::process_result::progress, ctx.ledger ().process (tx, *block).code);
				previous = block;
				auto receive = builder.make_block ()
							 ...
							 .previous (0)
							 .link (block->hash ())
							 .balance (nano::xrb_ratio)
							 .sign (key.prv, key.pub)
							 .work (*ctx.pool ().generate (key.pub))
							 .build_shared ();
				ASSERT_EQ (nano::process_result::progress, ctx.ledger ().process (tx, *receive).code);
			}
			ctx.store ().confirmation_height.put (tx, previous->account (), { previous->sideband ().height, previous->hash () });
		}
		std::filesystem::create_directory (snapshot);
		std::filesystem::copy (path, snapshot, std::filesystem::copy_options::recursive);
		return;
	}
	...
	nano::test::system system;
	{
		...
		for (auto rep : reps)
		{
			auto data_path = nano::unique_path ();
			std::filesystem::create_directory (data_path);
			std::filesystem::copy (snapshot, data_path, std::filesystem::copy_options::recursive);
			nano::node_config config;
			nano::node_flags flags;
			system.add_node (config, flags, nano::transport::transport_type::tcp, rep, data_path);
		}
	...
	auto log = [&] () {
		...
		if (last_log < now - std::chrono::seconds{ 1 })
		{
			last_log = now;
			for (auto i = 0; i < system.nodes.size (); ++i)
			{
				std::cerr << std::format ("{:2}({:6}) ", i, system.nodes[i]->ledger.cache.cemented_count.load ());
			}
			std::cerr << std::endl;
			//std::cerr << std::format ("c({})a({})i({})f({}) ", node->ledger.cemented_count (), node->stats.count (nano::stat::type::hinting, nano::stat::detail::activate), node->stats.count (nano::stat::type::hinting, nano::stat::detail::insert), node->stats.count (nano::stat::type::hinting, nano::stat::detail::insert_failed));
		}
	};
	ASSERT_TIMELY (3000s, (log (), std::all_of (system.nodes.begin (), system.nodes.end (), [&] (auto const & node) {
		return node->ledger.cache.cemented_count == node->ledger.cache.block_count;
	})));
	auto finish = std::chrono::steady_clock::now ();
	auto total = std::chrono::duration_cast<std::chrono::milliseconds> (finish - start).count ();
	auto work = std::chrono::duration_cast<std::chrono::milliseconds> (finish - prep).count ();
