...
TEST (system, receive_while_synchronizing)
{
	std::vector<boost::thread> threads;
	{
		...
		node1->workers.add_timed_task (std::chrono::steady_clock::now () + std::chrono::milliseconds (200), ([&system, &key] () {
			...
			auto transaction (system.nodes[0]->store.tx_begin_read ());
			auto block = system.nodes[0]->ledger.any.get (transaction, hash);
...
TEST (ledger, deep_account_compute)
{
	...
	for (auto i (0), n (100000); i != n; ++i)
	{
		...
		ledger.any.account (transaction, sprevious);
...
TEST (confirmation_height, many_accounts_single_confirmation)
{
	...
	ASSERT_TIMELY (120s, node->ledger.confirmed.exists_or_pruned (node->store.tx_begin_read (), last_open_hash));
...
TEST (confirmation_height, long_chains)
{
	...
	ASSERT_TIMELY (30s, node->ledger.confirmed.exists_or_pruned (node->store.tx_begin_read (), receive1->hash ()));
	...
	info = node->ledger.any.get (transaction, key1.pub);
	ASSERT_TRUE (info);
...
TEST (node, aggressive_flooding)
{
	...
	auto all_received = [&nodes_wallets] () {
		return std::all_of (nodes_wallets.begin (), nodes_wallets.end (), [] (auto const & node_wallet) {
			...
			return node_wallet.first->ledger.any.balance (node_wallet.first->store.tx_begin_read (), local_representative) > 0;
		});
	};
	...
	std::shared_ptr<nano::state_block> block{};
	{
		...
		block = builder.make_block ()
				.account (nano::dev::genesis_key.pub)
				.representative (nano::dev::genesis_key.pub)
				.previous (node1.ledger.any.head (transaction, nano::dev::genesis_key.pub))
				.balance (node1.ledger.any.balance (transaction, nano::dev::genesis_key.pub).value () - 1)
				.link (nano::dev::genesis_key.pub)
				.sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				.work (*node1.work_generate_blocking (node1.ledger.any.head (transaction, nano::dev::genesis_key.pub)))
				.build ();
	}
...
TEST (node, wallet_create_block_confirm_conflicts)
{
	for (int i = 0; i < 5; ++i)
	{
		...
		{
			auto block = node->ledger.any.get (node->store.tx_begin_read (), latest);
			node->scheduler.manual.push (block);
			std::shared_ptr<nano::election> election;
			ASSERT_TIMELY (10s, (election = node->active.election (block->qualified_root ())) != nullptr);
			election->force_confirm ();
		...
		ASSERT_TIMELY (120s, node->ledger.confirmed.exists_or_pruned (node->store.tx_begin_read (), latest) && node->confirming_set.size () == 0);
		done = true;
		t.join ();
	}
}
