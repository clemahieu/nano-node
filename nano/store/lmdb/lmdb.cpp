...
void nano::store::lmdb::component::upgrade_v22_to_v23 (store::write_transaction & transaction)
{
	...
	auto iterate_accounts = [this] (auto && func) {
		...
		auto it = typed_iterator<nano::account, nano::account_info_v22>{ store::iterator{ iterator{ env.tx (transaction), account_store.accounts_handle } } };
		auto const end = typed_iterator<nano::account, nano::account_info_v22>{ store::iterator{ iterator{} } };
...
void nano::store::lmdb::component::rebuild_db (store::write_transaction const & transaction_a)
{
	...
	for (auto const & table : tables)
	{
		...
		for (typed_iterator<nano::uint256_union, nano::store::lmdb::db_val> i{ store::iterator{ iterator{ env.tx (transaction_a), table } } }, n{ store::iterator{ iterator{} } }; i != n; ++i)
		{
		...
		for (typed_iterator<nano::uint256_union, nano::store::lmdb::db_val> i{ store::iterator{ iterator{ env.tx (transaction_a), temp } } }, n{ store::iterator{ iterator{} } }; i != n; ++i)
		{
	...
	// Pending table
	{
		...
		mdb_dbi_open (env.tx (transaction_a), "temp_table", MDB_CREATE, &temp);
		// Copy all values to temporary table
		for (typed_iterator<nano::pending_key, nano::pending_info> i{ store::iterator{ iterator{ env.tx (transaction_a), pending_store.pending_handle } } }, n{ store::iterator{ iterator{} } }; i != n; ++i)
		{
		...
		// Put values from copy
		for (typed_iterator<nano::pending_key, nano::pending_info> i{ store::iterator{ iterator{ env.tx (transaction_a), temp } } }, n{ store::iterator{ iterator{} } }; i != n; ++i)
		{
