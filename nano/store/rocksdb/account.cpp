...
auto nano::store::rocksdb::account::begin (store::transaction const & transaction, nano::account const & account) const -> iterator
{
	...
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (transaction), store.table_to_column_family (tables::accounts), val } } };
}

auto nano::store::rocksdb::account::begin (store::transaction const & transaction) const -> iterator
{
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (transaction), store.table_to_column_family (tables::accounts) } } };
}
