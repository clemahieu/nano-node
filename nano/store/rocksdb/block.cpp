...
{
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (transaction), store.table_to_column_family (tables::blocks) } } };
}
...
auto nano::store::rocksdb::block::begin (store::transaction const & transaction, nano::block_hash const & hash) const -> iterator
{
	...
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (transaction), store.table_to_column_family (tables::blocks), val } } };
}
