...
auto nano::store::rocksdb::confirmation_height::begin (store::transaction const & transaction, nano::account const & account) const -> iterator
{
	...
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (transaction), store.table_to_column_family (tables::confirmation_height), val } } };
}

auto nano::store::rocksdb::confirmation_height::begin (store::transaction const & transaction) const -> iterator
{
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (transaction), store.table_to_column_family (tables::confirmation_height) } } };
}

auto nano::store::rocksdb::confirmation_height::end () const -> iterator
{
	return iterator{ store::iterator{ rocksdb::iterator{} } };
}
