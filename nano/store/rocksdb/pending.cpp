...
auto nano::store::rocksdb::pending::begin (store::transaction const & transaction_a, nano::pending_key const & key_a) const -> iterator
{
	...
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (transaction_a), store.table_to_column_family (tables::pending), val } } };
}

auto nano::store::rocksdb::pending::begin (store::transaction const & transaction_a) const -> iterator
{
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (transaction_a), store.table_to_column_family (tables::pending) } } };
}
