...
auto nano::store::rocksdb::pruned::begin (store::transaction const & transaction_a, nano::block_hash const & hash_a) const -> iterator
{
	...
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (transaction_a), store.table_to_column_family (tables::pruned), val } } };
}

auto nano::store::rocksdb::pruned::begin (store::transaction const & transaction_a) const -> iterator
{
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (transaction_a), store.table_to_column_family (tables::pruned) } } };
}
