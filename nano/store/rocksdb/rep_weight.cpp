...
auto nano::store::rocksdb::rep_weight::begin (store::transaction const & txn_a, nano::account const & representative_a) const -> iterator
{
	...
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (txn_a), store.table_to_column_family (tables::rep_weights), val } } };
}

auto nano::store::rocksdb::rep_weight::begin (store::transaction const & txn_a) const -> iterator
{
	return iterator{ store::iterator{ rocksdb::iterator{ store.db.get (), rocksdb::tx (txn_a), store.table_to_column_family (tables::rep_weights) } } };
}
