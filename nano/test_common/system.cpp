...
std::shared_ptr<nano::state_block> nano::test::upgrade_epoch (nano::work_pool & pool_a, nano::ledger & ledger_a, nano::epoch epoch_a)
{
	...
	auto latest = ledger_a.any.head (transaction, account);
	auto balance = ledger_a.any.balance (transaction, account).value_or (0);
...
void nano::test::system::generate_rollback (nano::node & node_a, std::vector<nano::account> & accounts_a)
{
	...
	auto info = node_a.ledger.any.get (transaction, account);
	if (info)
	{
...
void nano::test::system::generate_receive (nano::node & node_a)
{
	std::shared_ptr<nano::block> send_block;
	{
		...
		{
			send_block = node_a.ledger.any.get (transaction, item->first.hash);
		}
	}
...
{
	nano::uint128_t balance = node_a.ledger.any.balance (transaction_a, account_a).value_or (0);
